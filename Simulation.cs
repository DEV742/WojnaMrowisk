...
namespace WojnaMrowisk
{
    class Simulation
    {
        ...
        void WriteCSV()
        {
            using (StreamWriter writer = new StreamWriter("ColoniesData.csv"))
            {
                writer.WriteLine($"{"ColonyID"},{"MaxAnthills"},{"MaxAnts"},{"TimeOfDeath/V"}");
                foreach (ColonyStatistics colStat in colStats)
                {
                    writer.WriteLine($"{colStat.colID},{colStat.maxAnthillsNum},{colStat.maxAntsNum},{colStat.timeOfDeath}");
                }
            }
            using (StreamWriter writer = new StreamWriter("AnthillsData.csv"))
            {
                writer.WriteLine($"{"anthillColID"},{"anthillID"},{"maxAnts"},{"size"},{"timeOfCreation"},{"timeOfDeath/V"}");
                foreach (AnthillStatistics ahStat in ahStats)
                {
                    writer.WriteLine($"{ahStat.ahColId},{ahStat.ahId},{ahStat.maxAntsNum},{ahStat.size},{ahStat.timeOfCreation},{ahStat.timeOfDeath}");
                }
            }
            using (StreamWriter writer = new StreamWriter("AntsData.csv"))
            {
                writer.WriteLine($"{"ColonyID"},{"anthillID"},{"isQueen"},{"MaxHP"},{"Damage"},{"visRange"},{"foodRange"},{"speed"},{"creationTime"},{"timeOfDeath/V"}");
                foreach (AntStatistics antStat in antStats)
                {
                    writer.WriteLine($"{antStat.colId},{antStat.ahId},{antStat.isQueen},{antStat.maxHealth},{antStat.maxDamage},{antStat.visRange},{antStat.foodRange},{antStat.speed},{antStat.timeCreated},{antStat.timeOfDeath}");
                }
            }
        }
    }
